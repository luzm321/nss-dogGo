    ...
    public class WalkerRepository : IWalkerRepository
    {
        ...
        public Walker GetWalkerById(int id)
        {
            using (SqlConnection conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                        SELECT w.Id, w.Name [Walker Name], w.ImageUrl, w.NeighborhoodId, n.Id [nId], n.Name [Neighborhood Name], wk.Date, wk.Duration, o.Name [Owner Name], d.Id [DogId], d.Name [Dog Name]
                        FROM Walker w
                        LEFT JOIN Neighborhood n
                        ON w.NeighborhoodId = n.Id
                        LEFT JOIN Walks wk 
                    ...
                    while (reader.Read())
                    {
                        ...
                        // If there is a Duration in the database:
                        if (!reader.IsDBNull(reader.GetOrdinal("Duration")))
                        {
                            walker.Walk = new Walk
                            {
                                Date = reader.GetDateTime(reader.GetOrdinal("Date")),
                                Duration = reader.GetInt32(reader.GetOrdinal("Duration")),
                                Owner = new Owner
